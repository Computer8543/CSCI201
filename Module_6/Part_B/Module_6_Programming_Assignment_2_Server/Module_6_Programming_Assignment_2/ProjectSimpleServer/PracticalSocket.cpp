    ...
    SocketAddress::AddressType atype)
{
    ...
    if (rtnVal != 0)
        //throw SocketException("getaddrinfo() failed", gai_strerrorW(rtnVal));
        throw SocketException("getaddrinfo() failed: " + to_string(rtnVal));
...
string SocketAddress::getAddress() const {
    void* numericAddress;
...
int findTCPServerSocketDesc(const char* localAddr, const char* localPort) {
    ...
    if ((result = getaddrinfo(localAddr, localPort, &addrCriteria,
        &servAddr)) != 0)
        throw SocketException("getaddrinfo() failed: " + to_string(result));
...
TCPServerSocket::TCPServerSocket(unsigned short localPort) {
    ...
    if (::bind(sockDesc, (sockaddr*)&localAddr, sizeof(sockaddr_in)) < 0) {
        throw SocketException("Bind failed (bind())");
    }
...
/*void UDPSocket::joinGroup(const string& multicastGroup) {
    ...
    //multicastRequest.imr_multiaddr.s_addr = InetPton(multicastGroup.c_str());
